...
func removeTestLogdbDir(fs vfs.FS) {
	if err := fs.RemoveAll(RDBTestDirectory); err != nil {
		panic(err)
	}
}

func getTestLogReaderWithoutCache(entries []pb.Entry) *logdb.LogReader {
	db := getNewLogReaderTestDB(entries, vfs.NewMem())
...
func TestRLLTFindConflict(t *testing.T) {
	...
	defer removeTestLogdbDir(vfs.NewMem())
