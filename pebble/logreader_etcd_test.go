...
func getNewLogReaderTestDB(entries []pb.Entry, fs vfs.IFS) raftio.ILogDB {
	db := getNewTestDB("db-dir", "wal-db-dir", fs)
	ud := pb.Update{
...
func getTestLogReader(entries []pb.Entry, fs vfs.IFS) (*logdb.LogReader, raftio.ILogDB) {
	db := getNewLogReaderTestDB(entries, fs)
	ls := logdb.NewLogReader(LogReaderTestClusterID, LogReaderTestNodeID, db)
...
func testLogReaderAppend(t *testing.T, fs vfs.IFS) {
	...
	for i, tt := range tests {
		...
		for ii, e := range tt.wentries {
			if e.Index == 6 {
				logdb.plog.Infof("going to check term for index 6")
			}
			term, err := s.Term(e.Index)
			if e.Index == 6 {
				logdb.plog.Infof("Term returned")
			}
			if err != nil {
